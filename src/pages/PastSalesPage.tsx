...
const PastSalesPage = () => {
  ...
  useEffect(() => {
    ...
    getPastPerformance({ limit: 7 }) // 可以替换为你需要的时间范围参数
  ...
  useEffect(() => {
    ...
    if (performanceTab === "week" && latestDate) {
      ...
      // 用数据库的 latestDate 回推 5 天
      const end = new Date(latestDate);
      const start = new Date(end);
      start.setDate(end.getDate() - 5); // 共7天
  ...
  useEffect(() => {
    ...
    if (performanceTab === "week" || performanceTab === "month" || performanceTab === "year") {
      params.limit = periodLength * 2; // 取本周期+上周期，和 revenue 逻辑一致
    } else if (performanceTab === "custom") {
      if (dateRange.from) params.start_date = format(dateRange.from, "yyyy-MM-dd");
      if (dateRange.to) params.end_date = format(dateRange.to, "yyyy-MM-dd");
    }
  ...
  const profitRatioDisplay =
    ...
    // 不需要用 prevPeriodProfit 和 profitRatio
            ...
            <Popover>
              ...
              <PopoverContent className="w-auto p-0" align="start">
                <Calendar
                  ...
                  numberOfMonths={2}
                  components={{ Caption: CustomCaption }} // 👈 添加这行
